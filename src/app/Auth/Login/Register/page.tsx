...
import { apiFetch, endpoints } from "@/lib/api"; // ⬅️ usa apiFetch (público)
...
export default function RegisterPage() {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      // prepara payload para el backend
      const payload: any = {
        name: data.name,
        email: data.email,
        password: data.password,
        role: data.role.toUpperCase(), // ⬅️ "DOCTOR" | "PATIENT"
      };
  ...
  // si ya hay sesión, envía a dashboard/login 
  useEffect(() => {
        ...
        {watchRole === "doctor" && (
          <div className="rounded-xl border p-4 bg-zinc-50 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input label="Precio por consulta (MXN)" type="number" step="0.01" {...register("price")} />
